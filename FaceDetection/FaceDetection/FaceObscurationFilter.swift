...
class FaceObscurationFilter: Filter {
    ...
    func detectFace(complete: @escaping (Bool, CIImage?) -> Void) {
        let startTime = Date()
        ...
        print("rohit check: Features: \(features)")
        
        // Build a pixellated version of the image using the CIPixellate filter
        guard let _ = PixellationFilter(inputImage: inputImage).outputImage else {
            // Failed to pixellate
            return complete(false, nil)
        }
        
        return complete(true, inputImage)
    }
